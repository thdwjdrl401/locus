...
| M-e2e-soak 통합 소크 | 1만 대 × 1Hz, 60분+ 무손실 | 남은 천장 = 부하 생성기 (서버 p95 24ms 여유) |
...
<details>
<summary><b>M-e2e-soak — 전 구간 통합 소크: 60분+ 무손실, 지속 천장은 부하 생성기</b> (2026-07)</summary>

그동안 구간별로 따로 검증했던 것을 전 구간(HTTP → Streams → TimescaleDB)으로 한 번에: 1만 대 × 1Hz, 66분, **유실 0**(양끝 정산 — k6 발신 ≈ 스트림 수신 ≈ 전량 적재, lag·pending·포이즌 0).

- 지속 처리량이 10k를 살짝 못 미친(~9.8k) 원인을 규명: 서버는 정상(p95 24ms·디스크 70%)인데 부하 머신이 포화(load 11.47 > 코어 10·메모리 고갈) — **천장은 파이프라인이 아니라 단일 부하 생성기**.
