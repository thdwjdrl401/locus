 ...
 * 텔레메트리 영속·조회.
 *
 * <p>M0는 단건 저장(순진하게 먼저). 멱등은 DB의 {@code UNIQUE(device_id, recorded_at)}로. 벌크 적재는 M2, 이력 커서 조회는 M7.
 *
 * <p>조회는 모두 {@code idx_telemetry_device_time(device_id, recorded_at)} 인덱스를 탄다. {@link
 * #findLatestPerDevice()} 는 *순진한* 상관 서브쿼리 — 데이터가 커지면 느려질 수 있고, 그게 M4 {@code LatestStateLookup}(최신상태
 * 캐시) 개선의 before 대상이다.
 */
public interface TelemetryRepository extends JpaRepository<Telemetry, Long> {
    ...
    /**
     * 디바이스별 최신 1프레임(관제 지도의 "전체 최신"). {@code UNIQUE(device_id, recorded_at)} 덕에 디바이스당 정확히 1행. naive
     * 상관 서브쿼리 — M4 캐시 도입 전의 기준 구현.
     */
    @Query(
