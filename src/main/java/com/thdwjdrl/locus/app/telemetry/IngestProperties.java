 ...
 *
 * <ul>
 *   <li>{@code direct}(기본) — 요청당 단건 저장 + Device 등록(insert-if-absent). 옛 M1 A0.
 ...
 * </ul>
 *
 * <p>나머지는 배치 거동 손잡이: 배치 크기·최대 지연·큐 용량과, Device 등록(insert-if-absent) 토글({@code device-upsert=false}
 * = 레지스트리 미기록).
 */
@Component
...
public class IngestProperties {
    ...
    /**
     * Device 등록(insert-if-absent) 수행 여부. {@code false} = 레지스트리 미기록. 이름 {@code device-upsert}는 옛
     * UPSERT 시맨틱의 잔재 — 측정 명령·문서 호환 위해 유지(rename은 alias 챙겨 별도, RISKS 잔가지).
     */
