...
개발 철학: **순진하게 구현 → 일부러 부하 → 문제 재현 → 진단 → 개선 → 측정으로 증명.**
...
- "느려질 것 같아서 안 했다"가 아니라 "재현해보니 X에서 병목, Y로 개선, Z만큼 빨라짐"으로 말한다. 효과 없던 시도도 기록한다.

### 3.2 순진하게 먼저 (계획서 §3)
- 처음엔 단건 insert·offset 페이징·새로고침 조회로 둔다. 측정으로 깨지는 지점을 본 뒤 개선한다.
- 각 마일스톤의 "일부러 만드는 문제"를 건너뛰지 않는다. 그게  스토리의 근거다.
