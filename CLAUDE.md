...
| 무엇 | 어디 |
|---|---|
| **진행 현황 체크리스트(기준 문서)** | `docs/STATUS.md` |
...
**모든 작업·결정은 `docs/STATUS.md`에 반영한다.** 이게 진행 현황의 **유일한 기준**이다. 이 규칙은 기본동작을 덮어쓴다 — 빠뜨리지 말 것.

- **작업 완료 시**: 해당 체크박스를 `[x]`로, 상태 마커(✅/🔄/⬜)를 갱신. 없던 작업이면 항목을 추가하고 체크.
- **착수 시**: 시작하는 항목을 🔄로, **"현재 포커스"** 를 지금 상태로 갱신.
- **결정 시**: 되돌리기 싼 **보류 결정** → `docs/ROADMAP.md` "보류된 결정" 표 + STATUS에 기록. **확정 결정** → 해당 문서(ADR/STRUCTURE/conventions) + STATUS 반영. 결정은 *결정 시점 + 가벼운 가드레일*만(과잉결정 금지, §3.3).
