...
**기각 — TTL 만료로 유령 청소:** 조직 이동 유령과 정당한 오프라인은 옛 파티션 입장에서 둘 다 "갱신 멈춘 엔트리"라 TTL이 구분하지 못한다. TTL로 유령을 지우면 오프라인 디바이스도 같이 지워져 위 요구를 위반한다. 따라서 유령 청소는 명시적 HDEL(스펙 #2), 오프라인은 나이 파생으로 각각 처리한다.
...
원칙: **캐시는 지도에서 한눈에(at-a-glance) 필요한 것만, 상세는 요청 시 DB에서.** 근거는 (1) 스냅샷 크기 — 접속 시 조직 전체를 HGETALL로 당기므로 값이 얇을수록 스냅샷·push 델타가 싸다, (2) PII 최소화 — 캐시에 담긴 만큼 Redis 영속화·백업에 묻어간다.
