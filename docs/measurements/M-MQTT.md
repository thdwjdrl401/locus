...
**인입을 단일 콜백에서 바운드 익스큐터(워커 8)로 오프로드하니 3.25K→7.4K(2.3배)로 오르고, 처리량이 2.3배인데 디스크는 90→80%로 낮아졌다.** 인과 사슬을 그대로 되짚는다: 인입↑ → 스트림이 차서 배치 28→156행 → 커밋 102→48/s → fsync↓ → 디스크 여유. "디스크 90%는 저인입 증상"이 개선으로 확증됐다. `worker-threads=0`(인라인)과 8의 토글로 한 변수만 바꿔 대조([raw](M-MQTT-raw/sustain3-worker8/)).

| 지표 | worker 0 (before) | worker 8 (after) |
|---|---|---|
| 인입 received/s | 3,250 | **7,357** |
| 배치 행/flush | 28 | 156 |
| 커밋/s | 102 | 48 |
| 디스크 %util | 90% | 80% |
| 브로커 drop/s | 6,700 | 2,720 |

- 매뉴얼 ack(XADD 성공 후 ack)로 at-least-once 유지, 큐 가득참=CallerRuns 백프레셔. 내부 무손실(received=inserted=DB, dropped=0). OOM 없음(스왑 off·가용 1.9G, 워커 8 + 연결 1만이 8GB 안).

### 다음 — 단일 구독 연결 (미측정)
worker 8은 7.4K에서 평평했고 디스크는 80%로 여유가 있다. HTTP(9.7K)와의 갭은 디스크가 아니라 **단일 구독 연결의 QoS1 인플라이트 창**이 파이프 폭을 제한하는 것으로 보인다(스레드는 풀렸으나 브로커→구독자 연결이 하나). 다음 슬라이스: 다중 구독 연결 또는 inflight 상향(ADR 0004의 HiveMQ 전환 게이트와 연결). before/after 측정으로 확인.
