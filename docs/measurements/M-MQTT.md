...
### 결론
단일 구독 콜백 스레드가 인입을 묶는다는 진단이 두 병렬화 슬라이스로 검증됐다. ~9K에서 디스크 80%·HTTP 천장 근접 → **인입은 더 이상 병목이 아니다.** 남은 갭(~9K vs 9.7K)은 스토리지/디스크 실제 천장이지 인입 계층이 아니다. 더 짜려면 스토리지 축(디스크·배치·워커)이 대상이고, 그건 M2-par/M4b가 이미 다룬 영역이다.
