...
- **결정적 지표로 인과를 증명한다.** 처리량만 보면 "빨라졌다"까지고, 지표(fsync/req, 디스크 읽기량, `EXPLAIN` loops, `accepted−DB` 유실, `XLEN` vs `MAXLEN`)가 함께 움직여야 왜 빨라졌는지가 증명된다. 잘못 해석한 사례도 기록했다 — M2의 k6 타임스탬프 미래 표류를 서버 붕괴로 잘못 해석한 일, M2-par에서 램프 피크를 SLO 달성으로 조기 선언한 일, M4b에서 90s 런의 warm-up 트랜지언트(6k)를 지속 최대 처리량으로 잘못 읽었다가 5분 런(정상상태 10k)으로 바로잡은 일.
