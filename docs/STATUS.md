...
| **✅ 표현 규칙 추가 + 적용 (2026-08-15)** | conventions §7에 **"같은 수사 구문을 반복하지 않는다"** 항목 신설 — 특히 `X가 아니라 Y다` 대조 구문은 귀속이 실제로 바뀔 때만, **한 절에서 2회 이내**. 계기: PERFORMANCE ⑧~⑩ 초안을 같은 문서의 기존 절(①~⑦)과 같은 지표로 대조하니 이 구문이 100문장당 **25.8회 vs 7.5회(3.4배)** 로 과했다. 다른 AI 티 지표(피동·무생물 주어·`-들`·대명사)는 양쪽 다 0이고 종합 점수·경로 판정도 동일 → 문제는 이 구문 하나로 좁혀짐. 적용 결과 ⑧~⑩ 25.8→**8.5회**, 전 절이 2회 이내(귀속 반전인 자리만 남기고 나머지는 평서문으로). 자가 점검법도 규칙에 기재 |
| **✅ PERFORMANCE 서사 현행화 (2026-08-15)** | 병목 이동 서사가 ⑦ M4b에서 끊겨 있던 것을 현재까지 이었다(2026-07-13 로그의 "잔여(별도)" 항목 해소). 추가: **⑧ M-MQTT**(수집 어댑터 인입 3.25K→~9K, 디스크 90%는 저인입 증상이었음을 HTTP 대조군으로 가림, 다음 병목=mosquitto 단일 스레드) · **⑨ M-http-capacity**(코어 핀 6→8에 12K→16K 선형 확장=CPU 바운드 확증, 스레드·큐락·Poller·스토리지 반증, ⑧의 HTTP 귀속 정정) · **⑩ M-e2e-soak**(전 구간 61분 무손실 양끝 정산, 지속 한계=단일 부하 생성기, 가상 스레드 회귀 기각) · **◆ disk-baseline**(마일스톤 아닌 하드웨어 특성화 — ④의 "순차 대역폭 수렴" 귀속을 정정: 대역폭은 0.26%만 쓰고 한계는 durable sync 지연·헤드 경합. `%util`≠포화). 병목 이동 지도 표·한 줄 요약·관통하는 원칙(오판 목록·대조군 원칙·폐기한 시도)·상세 문서 표 동반 갱신 |
...
| 2026-08-11 | **조직 배정을 수집 경로 설정으로 임시 개방**(`locus.ingest.default-org`, 기본 null) — org 없는 디바이스는 조회·push·지오펜스에서 전부 제외되는데 org를 채우는 코드가 없어 신규 DB에서 관제 화면이 비었다. 생성 시 1회만 배정하고 기존 org는 보존. 최종형 아님 — enrollment(인증)가 이 자리를 대체한다. 시뮬레이터 프로파일이 `org-0` 제공 | `IngestProperties`, `DirectIngestWriter`, `TelemetryBatchDao`, `application-simulator.yml`, `DeviceOrgScopeIntegrationTest` |
...
| 2026-07-10 | **병목 진단 사슬 정정 + disk-baseline 커밋** — fio HDD 특성화(`disk-baseline.md`) 커밋. **진단 정련: M2-par(디스크 대역폭) → disk-baseline(fsync/seek 지연·대역폭 0.26%만 씀) → M-http-capacity(배치 운영점=CPU 바운드)**. regime 구분이 핵심(naive per-commit=디스크 fsync 벽, 배칭이 amortize→CPU가 다음 벽). 함의: disk-baseline "SSD=유일 큰 레버"는 fsync-바운드 한정으로 톤 조정(배치 12–16K 천장은 CPU라 SSD 무영향), `%util≠포화` 재확인. device 데모션(R5a) **perf 명분 소멸**도 여기서 귀결 | [disk-baseline.md](measurements/disk-baseline.md), [M-http-capacity.md](measurements/M-http-capacity.md), [RISKS.md](RISKS.md) |
| 2026-07-09 | **아키텍처 리스크 등록부 신설(`docs/RISKS.md`)** — 수집 파이프라인 구조 평가. 열린 위험 정리: R1 Redis 장애 시 버퍼·offset 유실(내구가 앱 크래시까지만, 미결·높음), R2 지속 과부하 조용한 트림 유실+admission 부재("202=저장 보장"이 ≤storage 용량 조건, 미결), R3 CPU 경합·단일박스 포화(M8 연기), R4 컨슈머 용량 수동·부족 시 조용한 유실(geofence가 사례), R5 잔가지(device upsert·PEL·JSON). ADR=결정/ROADMAP=미룬결정과 구분되는 열린위험 축 | [RISKS.md](RISKS.md) |
| 2026-07-09 | **stream 적재 워커 스윕 연장 — 워커4→8로 10K→12K 무손실** — M4b는 10K SLO에서 멈췄는데, 8코어(0-7)·stream·워커8로 밀어보니 적재가 인입 12K와 매칭(storage lag 0·pending 0). ~10K는 디스크 벽이 아니라 **워커4 병렬화 부족**(disk %util 77%로 여유였고 8워커가 빈 사이클 채워 86%·큐 3.5). **이번 세션 오진 정정**: "10K=디스크 대역폭 벽"은 틀렸고 `%util`로 saturation 판단한 게 오류 — 실제 신호는 flush 지연(20→190ms)·큐깊이. 다음 벽=CPU(98%)+SSD. 비교주의: 8코어/fleet라 M4b 10K(workers4/capacity/코어0-5)와 apples-to-apples 아님 | [M4b.md §후속](measurements/M4b.md) |
