...
| **다음 한 걸음** | **측정 준비 중(2026-07-03)**: 첫 슬라이스(이음새·핸들러·통합테스트) 완료에 이어 ⓐ 수신/드롭 카운터(`locus.mqtt.received/dropped` — 측정 오라클, 통합테스트 단언 포함) ⓑ Grafana `locus-mqtt` 대시보드 ⓒ **토픽 재설계 `telemetry/{deviceId}`**(결정 로그 참조, 통합테스트 4개로 재작성). **측정 계획**: 런1 램프 2k→10k(현재 코드=Paho 단일 콜백 스레드 인라인, 병목 위치 확인) → 막히면 런2 익스큐터 오프로드 → 런3 flat 10k 5분(무손실 오라클=received−dropped==DB). 부하=emqtt-bench(Docker, 맥). **스모크 완료(2026-07-03)**: `%TIMESTAMP%`=밀리초·메시지마다 갱신 확인, 토픽 변수는 레벨 전체일 때만 치환(`telemetry/%i` ✓, `bench-%i` ✗ → 벤치 deviceId=seqno 또는 `%c`). 숫자 timestamp는 Jackson 기본이 초 해석이라 전량 드롭이었을 것 → **epoch-ms 해석으로 결정**(결정 로그). **함정 반영**: Mosquitto `max_queued_messages`(기본 1000) 드롭 관측($SYS), 단일 구독 연결이 구조적 직렬화 지점(shared subscription은 MQTT5=HiveMQ 게이트). |
