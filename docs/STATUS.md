...
| 2026-07-05 | **stream storage poison 내성** — payload null(트림된 유령 pending)·손상 JSON이 재시작 pending 회수(`recoverPending`)에서 워커 스레드를 죽이던 버그. 처리 불가 엔트리는 드롭(XACK)+`locus.ingest.poison` 카운트(안 버리면 재시작마다 무한 재처리), 좋은 엔트리만 적재 후 XACK. 시뮬레이터 데모 중 stale 스트림으로 재현·CI 통합테스트로 못박음 | `StreamStorageConsumer`, `StreamIngestIntegrationTest` |
...
- [x] **stream storage poison 내성** (2026-07-05) — payload null(MAXLEN 트림된 유령 pending)·손상 JSON 엔트리가 재시작 pending 회수에서 워커 스레드를 죽이던 버그 수정. `tryRead`가 던지지 않고 드롭(XACK+로그+`locus.ingest.poison` 카운트), 좋은 엔트리만 적재 후 XACK(at-least-once 유지). 통합테스트 동반(실 Redis에 poison XADD→워커 생존+정상 적재 검증). 시뮬레이터 데모 중 stale 스트림으로 재현
### 인증(별도 — 측정 주도 아님)
