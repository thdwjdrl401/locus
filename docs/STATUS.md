...
| **다음 한 걸음** | **측정 완료 — 병목 규명(2026-07-03)** → [measurements/M-MQTT.md](measurements/M-MQTT.md). **MQTT 단일 Paho 콜백 스레드가 인입을 ~3.25K/s로 묶는다. 같은 stream 스토리지가 HTTP로는 ~9.7K/s(디스크 65%)를 같은 날 처리 → 병목은 적재가 아니라 수집 어댑터의 인입(3배 차).** 저인입이라 스토리지가 28행 작은 배치로 커밋 남발(~100/s ≈ HDD fsync 천장) → 디스크 90%는 스토리지 한계가 아니라 저인입 증상(대조군 HTTP로 격리). 내부 무손실(received=inserted=DB, dropped=0)·연결 1만 유지·초과분 브로커 드롭. **런2 측정 완료(2026-07-04)**: 인입 오프로드(`worker-threads` 0→8)로 **인입 3.25K→7.4K(2.3배)**, 처리량 2.3배인데 디스크 90→80%로 낮아짐(배치 28→156행·커밋 102→48/s·브로커drop 6.7K→2.7K). "디스크 90%=저인입 증상" 개선으로 확증. at-least-once 유지·내부 무손실·OOM 없음(8GB). **다중 연결 슬라이스 구현(2026-07-04, 측정 대기)**: `connections` 토글(1=단일 현행, N=N개 Paho 클라이언트가 **shared subscription** `$share/{group}/telemetry/+`으로 분배 수신). worker8 7.4K 평평·디스크 80% 여유 → 갭은 단일 연결 QoS1 인플라이트/수신 스레드. 통합테스트 `MqttSharedSubscriptionIntegrationTest`가 shared-sub 분배(received≈R×N, 2배 아님) 검증 — **Mosquitto+Paho3 shared-sub 작동 여부가 CI 게이트**. CI green 후 박스에서 connections 1→2→4 측정(7.4K→9.7K 회복하나). 상세 [M-MQTT.md](measurements/M-MQTT.md). 부하도구 함정(template:// 파일모드·`%TIMESTAMPMS%`·`--ulimit`·`docker kill`·브로커 nofile 1M)은 `load/mqtt-*`·M-MQTT.md에 기록. |
...
| **M-MQTT** | **MQTT 수집 경로** | 🔄 병목 규명 + 런2 개선 실증(인입 오프로드 3.25K→7.4K·2.3배, 디스크 90→80%). 다음=단일 구독 연결(7.4K 평평·9.7K 갭) | MQTT | Mosquitto |
