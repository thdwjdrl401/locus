...
| **다음 한 걸음** | **측정 프로토콜 확정(2026-07-03)** → [measurements/M-MQTT.md](measurements/M-MQTT.md). 준비 ⓐ 수신/드롭 카운터(`locus.mqtt.received/dropped`, 통합테스트 단언) ⓑ Grafana `locus-mqtt` ⓒ 토픽 `telemetry/{deviceId}` 모두 완료. **측정 = `mode=stream` 고정**(목표 경로·하류는 M4b 무손실 특성화라 변수를 인입 계층으로 좁힘). **손실 오라클 = 3경계 정산**(브로커 도달 R vs 발행 P / 어댑터 mqtt.dropped / 스트림 트림·dedup) — `received−dropped==DB`는 stream·QoS1·MAXLEN 400K로 dedup·트림 0일 때만 성립, 브로커 경계 손실은 별도 귀속. 런1 램프 2k→10k(단일 콜백 스레드 인라인, 병목 위치) → 막히면 런2 익스큐터 오프로드 → 런3 flat 10k 5분. 부하=emqtt-bench(1만 디바이스×1Hz, Docker/맥). **측정 준비 거의 완료**: 대시보드 stream 적재 패널 + 부하 스크립트(`load/mqtt-{ramp,sustain}.sh`·payload 템플릿) 확정. **부하 함정 3개 발견·해결(2026-07-03)**: ⓐ emqtt-bench 템플릿 변수는 `template://` 파일 모드에서만 치환(인라인 `-m` 리터럴은 전량 드롭) ⓑ `%TIMESTAMP%`(초)가 아니라 `%TIMESTAMPMS%`(ms) — 앱이 epoch-ms로 읽어 초값은 `@ValidTimestamp` 드롭 ⓒ `--ulimit nofile`은 `docker run`에(호스트 ulimit 미전파 → `emfile`) ⓓ 시간 제어는 `docker kill`(macOS엔 GNU `timeout` 없음) ⓔ 브로커 FD도 상향 — mosquitto 컨테이너 기본 nofile ~1024면 커넥션 ~1024 위는 `tcp_closed` 거절, compose `ulimits.nofile`로 1M(발행자·브로커 양쪽 다 풀어야 1만 붙음). 관측 스크립트 `scripts/mqtt-observe.sh`(sys·stream·vmstat·snap → `M-MQTT-raw/<RUN>/`) 확정. **예행 통과**: 치환 실증(`%TIMESTAMPMS%`→13자리 ms·메시지마다 유일=dedup 0), 1만 커넥션 붙음. 남은 것 — 관측 켜고 본 측정 런, 이미지 digest 핀. **스모크 완료(2026-07-03)**: `%TIMESTAMP%`=밀리초·메시지마다 갱신 확인, 토픽 변수는 레벨 전체일 때만 치환(`telemetry/%i` ✓, `bench-%i` ✗ → 벤치 deviceId=seqno 또는 `%c`). 숫자 timestamp는 Jackson 기본이 초 해석이라 전량 드롭이었을 것 → **epoch-ms 해석으로 결정**(결정 로그). **함정 반영**: Mosquitto `max_queued_messages`(기본 1000) 드롭 관측($SYS), 단일 구독 연결이 구조적 직렬화 지점(shared subscription은 MQTT5=HiveMQ 게이트). |
...
| **M-MQTT** | **MQTT 수집 경로** | 🔄 첫 슬라이스 완료(통합테스트 green)·측정 프로토콜 확정(stream 모드)·측정 예정 | MQTT | Mosquitto |
