...
- **같은 수사 구문을 반복하지 않는다.** 특히 **"X가 아니라 Y다"** 대조 구문은 귀속이 실제로 바뀔 때만 쓴다(예: "병목은 대역폭이 아니라 sync 지연이다" — 앞선 귀속을 측정으로 뒤집은 자리). 같은 산문 절에서 되풀이하면 강조가 아니라 습관으로 읽힌다. 나머지는 평서문으로 쓴다("한계는 durable sync 지연이다", "지배분은 앱 요청 처리다").
  - **대상은 `아니라` 대조 구문**이다. 단순 부정(`~가 아니다`·`병목이 아님`)은 해당 없음 — 반증 기록·기각 사유는 이 프로젝트의 방법 자체라 줄일 이유가 없다.
  - **세는 단위는 산문 절**이다. 표·변경 이력(STATUS 결정 로그)·반증 목록처럼 항목마다 독립인 곳은 제목 하나로 묶여도 절로 세지 않는다.
  - 자가 점검: 산문 절 하나에서 **2회 이내**. `grep -c '아니라' <파일>`을 문장 수로 나눠 밀도로 본다(작성 시점 기록: PERFORMANCE ⑧~⑩ 초안이 100문장당 25.8회로 기존 절 7.5회의 3.4배 → 정리 후 8.5회).
